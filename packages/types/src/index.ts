export interface MediaVariantURL {name:string;format:string;url:string;width?:number;height?:number}
export interface MediaImage {url:string;alt:string;width?:number;height?:number;caption?:string;credit?:string;copyrightNotice?:string;variants?:MediaVariantURL[]}
export type ContentKind = 'ACTIVITY' | 'PROJECT' | 'PUBLICATION';
export interface ContentRecord { id: string; slug: string; kind: ContentKind; locale: string; title: string; summary: string; body: string; category: string; isDemo: boolean; publishedAt: string | null; image?: MediaImage | null; status?:string|null; startDate?:string|null; endDate?:string|null; seoTitle?:string|null; seoDescription?:string|null; }

export interface Partner {id:string;name:string;slug:string;logo?:MediaImage;website?:string;description:string;country:string;partnerType:'Institutional Partner'|'Academic Partner'|'Research Partner'|'Public Authority'|'International Organisation'|'Local Authority'|'Strategic Partner';featured:boolean;sortOrder:number;published:boolean}
