...
export class FormsController {
  ...
  @ApiResponse({
    status: 201,
    description: 'Enquiry stored and notification queued.',
  })
  async contact(@Body() dto: ContactDto, @Req() req: Request) {
    const { website, acknowledgement, ...data } = dto;
    const secret =
      this.config.get<string>('CONTACT_RATE_SECRET') ||
      this.config.get<string>('ADMIN_API_TOKEN') ||
      'local-development-only';
    const hash = (s: string) =>
      createHmac('sha256', secret).update(s).digest('hex');
    const ip = contactClientIp(req, this.config.get('CONTACT_PROXY_SECRET'));
    ...
    const reference = await this.db.$transaction(async (tx) => {
      ...
      if (prior && now - prior.at < 600000) return prior.reference;
      for (const [key, ttl, limit] of [
        ['contact-rate:' + hash('email:' + dto.email), 3600000, 3],
        ['contact-rate:' + hash('ip:' + ip), 60000, 5],
      ].sort((a, b) => String(a[0]).localeCompare(String(b[0]))) as [
        string,
        number,
        number,
      ][]) {
      ...
      const row = await tx.contactSubmission.create({
        data: { ...data, consentVersion: 'privacy-notice-2026-09' },
      });
      for (const kind of ['notify', ...(acknowledgement ? ['ack'] : [])]) {
        const key = 'contact-mail:' + row.id + ':' + kind;
      ...
      await tx.siteSetting.upsert({
        where: { key: dedupe },
        create: { key: dedupe, value: { reference: row.id, at: now } },
        update: { value: { reference: row.id, at: now } },
      });
      await tx.siteSetting.deleteMany({
        where: {
          OR: [
            { key: { startsWith: 'contact-rate:' } },
            { key: { startsWith: 'contact-dedupe:' } },
          ],
    ...
    void this.mail.processPending().catch(() => {});
