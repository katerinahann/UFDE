          ...
          return [
            {
              ...
              destination:
                (process.env.NEWSLETTER_BACKEND_URL ||
                  'http://localhost:4000/v1/newsletter') + '/subscribe',
            },
            {
              source: '/api/admin/:path*',
              destination:
                (process.env.ADMIN_BACKEND_URL ||
                  'http://localhost:4000/v1/admin') + '/:path*',
            },
            {
              source: '/api/media/:id',
              destination:
                ...
                ).replace(/\/admin\/?$/, '') + '/media/:id',
            },
            {
              source: '/api/contact',
              destination:
                process.env.CONTACT_BACKEND_URL ||
                'http://localhost:4000/v1/contact',
            },
          ];
        },
        async headers() {
