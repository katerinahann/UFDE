...
export async function About({locale}:{locale:Locale}){
 const areas=await getStrategicAreas(locale);
 ...
 <Section><Container><div id="mission-values" className="about-two-column about-mission"><div><SectionHeading title={a.missionTitle} underline/>{a.mission.map((p,i)=><p key={p} className={i===0?'about-lead':undefined}>{p}</p>)}</div><AboutPhotoCarousel photos={profile.photos} label={a.carousel} previous={a.previous} next={a.next} slide={a.slide}/></div></Container></Section>
 <Section tone="soft"><Container><div id="our-story" className="about-two-column"><div><SectionHeading title={a.whoTitle} underline/>{a.who.map(p=><p key={p}>{p}</p>)}</div><dl className="about-facts">{a.factLabels.map((label,i)=><div key={label}><dt>{label}</dt><dd>{facts[i]}</dd></div>)}</dl></div></Container></Section>
 ...
 <Section tone="soft"><Container><SectionHeading title={d.home.strategic} underline/><div className="home-strategic-grid about-strategic">{areas.map(({title,description,slug},i)=>{const Icon=areaIcons[i%6];return <StrategicAreaCard key={title} title={title} description={description} icon={<Icon/>} href={href('/strategic-areas/'+slug)} linkLabel={d.home.learn}/>})}</div></Container></Section>
 <PartnerSection locale={locale}/><Container><div className="about-credits"><p>{a.imageNote}</p>{profile.photos.map(p=><p key={p.url}>{a.photoCredit}: <a href={p.source}>{p.alt} — {p.credit}</a> · <a href={p.licenseUrl}>{p.licenseLabel}</a> · {d.cropped}</p>)}</div></Container></div>;
}
