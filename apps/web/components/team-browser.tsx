...
export function TeamBrowser({members,locale}:{members:TeamProfile[];locale:Locale}){
 ...
 return <div className="team-browser"><h2 className="team-section-label">{groups.find(g=>g.value===category)?.label||l.leadership}</h2>{groups.length===1?<TeamRows members={members.filter(m=>m.category==='leadership')} locale={locale}/>:<CategoryTabs value={category} onValueChange={setCategory} label={l.title} items={groups.map(g=>({...g,content:<TeamRows key={g.value} members={members.filter(m=>m.category===g.value)} locale={locale}/>}))}/>}</div>;
}
