...
import {getPartners,getAboutProfile} from './content';
...
else if(key==='partners')body=<PartnerStrip partners={await getPartners()} locale={locale} grid/>;
...
else if(key==='team'){let rows:Awaited<ReturnType<typeof getTeam>>=[];let failed=false;try{rows=await getTeam(locale)}catch{failed=true}body=rows.length?<div className="team-grid">{rows.map(r=><article key={r.id} className="team-card">{r.photoUrl&&safeImage(r.photoUrl)&&<Image src={safeImage(r.photoUrl)!} alt={r.photoAlt||r.name} width={400} height={500}/>}<h2>{r.name}</h2><p className="category">{r.role}</p><p>{r.biography}</p></article>)}</div>:<div className="empty-state"><Users size={36}/><h2>{failed?d.unavailable:blocks[3]}</h2><p>{blocks[4]}</p><Link className="text-link" href={href('/contact')}>{d.connect[3]}<ArrowRight size={17}/></Link></div>;}
