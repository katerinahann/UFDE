...
export async function routeMetadata(locale:Locale,props:RouteProps){const {path=[]}=await props.params;const d=dictionary(locale);if(!path.length)return metadataFor(locale,'',d.fullName,d.home.description,institute.hero.src);if(path[0]==='publications'&&path.length<=2){const p=path[1]?await getPublication(path[1],locale):null;if(path[1]&&!p)notFound();return metadataFor(locale,'/'+path.join('/'),p?.title||publicationLabels[locale].title,p?.summary||publicationLabels[locale].subtitle,p?p.coverImage?.url:'/images/sorbonne.webp');}if(path.length===2&&kinds[path[0]]){const item=await getContent(path[1],locale,kinds[path[0]]);if(!item||item.kind!==kinds[path[0]])notFound();return metadataFor(locale,'/'+path.join('/'),item.seoTitle||item.title,item.seoDescription||item.summary,item.image?.url);}
