...
import {ArrowRight,ArrowLeft,FileText,Mail,MapPin,Users,BookOpen} from 'lucide-react';
...
import {serializeJsonLd} from './seo-data';
...
export async function routeMetadata(locale:Locale,props:RouteProps){const {path=[]}=await props.params;const d=dictionary(locale);if(!path.length)return metadataFor(locale,'',d.fullName,d.home.description,institute.homeHero.src);if(path[0]==='publications'&&path.length<=2){const p=path[1]?await getPublication(path[1],locale):null;if(path[1]&&!p)notFound();return metadataFor(locale,'/'+path.join('/'),p?.title||publicationLabels[locale].title,p?.summary||publicationLabels[locale].subtitle,p?p.coverImage?.url:'/images/sorbonne.webp');}if(path.length===2&&kinds[path[0]]){const item=await getContent(path[1],locale,kinds[path[0]]);if(!item||item.kind!==kinds[path[0]])notFound();return metadataFor(locale,'/'+path.join('/'),item.seoTitle||item.title,item.seoDescription||item.summary,item.image?.url);}
...
else if(key==='contact')body=<div className="contact-layout"><aside><Eyebrow>{blocks[3]}</Eyebrow><h2>UFDE</h2><p>{d.fullName}</p><div className="contact-details"><p><MapPin size={20}/>{institute.location}</p><a href={'mailto:'+institute.email}><Mail size={20}/>{institute.email}</a></div></aside><section><h2>{blocks[4]}</h2><EnquiryForm locale={locale} d={d.form} demo={demoMode}/></section></div>;
...
return <><Header locale={locale} path={fullPath}/><main id="main">{content}</main><PreviewNotice locale={locale}/><Footer locale={locale}/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:serializeJsonLd(await structuredPage(locale,fullPath,title,description))}}/></>;
}
