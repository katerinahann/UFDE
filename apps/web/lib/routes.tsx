...
else if(key==='partners'&&path.length===1){title=await getPartnerLabel(locale);description=d.pages.partners[2];content=<><section className="page-heading"><div className="container"><h1>{title}</h1></div></section><section className="container section"><PartnerStrip partners={await getPartners()} locale={locale} label={title} grid/></section></>;}
