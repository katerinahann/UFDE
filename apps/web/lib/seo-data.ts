...
export function personSchema(origin:string,p:TeamProfile){return {'@type':'Person','@id':origin+'/team#'+encodeURIComponent(p.id),name:p.name,jobTitle:p.role,description:p.biography,image:absoluteUrl(p.photoUrl,origin),sameAs:[p.linkedin,p.orcid,p.googleScholar,p.institutionalProfile].map(u=>absoluteUrl(u,origin)).filter(Boolean)};}
